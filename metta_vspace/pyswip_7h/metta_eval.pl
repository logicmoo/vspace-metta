...
eval_args(RetType,_Dpth,_Slf,X,Y):- nonvar(Y),X=Y,!.
eval_args(RetType,Depth,Self,X,Y):- nonvar(Y),!,eval_args(RetType,Depth,Self,X,XX),evals_to(XX,Y).
eval_args(RetType,_Dpth,_Slf,X,Y):- self_eval(X),!,Y=X.
eval_args(RetType,_Dpth,_Slf,[X|T],Y):- T==[], \+ callable(X),!,Y=[X].

eval_args(RetType,Depth,Self,[F|X],Y):-
  (F=='superpose' ; ( option_value(no_repeats,false))),
  mnotrace((D1 is Depth-1)),!,
  eval_args111(RetType,D1,Self,[F|X],Y).

eval_args(RetType,Depth,Self,X,Y):-
  mnotrace((no_repeats_var(YY),
  D1 is Depth-1)),
  eval_args111(RetType,D1,Self,X,Y),
   mnotrace(( \+ (Y\=YY))).
...
eval_args111(RetType,Depth,_Slf,X,Y):- Depth<1,!,X=Y, (\+ trace_on_overflow-> true; flag(eval_num,_,0),debug(metta(eval))).
eval_args111(RetType,_Dpth,_Slf,X,Y):- self_eval(X),!,Y=X.
eval_args111(RetType,Depth,Self,X,Y):-
  Depth2 is Depth-1,
  eval_args11(RetType,Depth,Self,X,M),
  (M\=@=X ->eval_args111(RetType,Depth2,Self,M,Y);Y=X).
...
eval_args11(RetType,Depth,Self,X,Y):- flag(eval_num,EX,EX+1),
  ...
  (Ret\=@=retval(fail)->true;(rtrace(eval_args111(RetType,D1,Self,X,Y)),fail)).
...
eval_args20(RetType,Depth,Self,[V|VI],[V|VO]):- var(V),is_list(VI),!,maplist(eval_args(RetType,Depth,Self),VI,VO).
...
eval_args20(RetType,Depth,Self,['match',Other,Goal,Template,Else],Template):-
  (eval_args20(RetType,Depth,Self,['match',Other,Goal,Template],Template)*->true;Template=Else).
...
eval_args20(RetType,Depth,Self,X,Res):-
  ...
  best_key(AA,Cases,Value):-
   ...
   maybe_special_keys(Depth,Self,[K-V|KVI],[AK-V|KVO]):-
     eval_args(RetType,Depth,Self,K,AK), K\=@=AK,!,
...
%[collapse,[1,2,3]]
eval_args20(RetType,Depth,Self,['collapse',List],Res):-!, bagof_eval(Depth,Self,List,Res).
%[superpose,[1,2,3]]
eval_args20(RetType,Depth,Self,['superpose',List],Res):- !, member(E,List),eval_args(RetType,Depth,Self,E,Res).
...
eval_args20(RetType,Depth,Self, Term, Res):-
...
eval_args20(RetType,Depth,Self,['if',Cond,Then],Res):- !,
   eval_args(RetType,Depth,Self,Cond,TF),
...
eval_args20(RetType,Depth,Self,['let',A,A5,AA],OO):- !,
  %(var(A)->true;trace),
  ((eval_args(RetType,Depth,Self,A5,AE), AE=A)),
...
eval_args20(RetType,_Dpth,_Slf,['car-atom',Atom],CAR):- !, Atom=[CAR|_],!.
eval_args20(RetType,_Dpth,_Slf,['cdr-atom',Atom],CDR):- !, Atom=[_|CDR],!.

eval_args20(RetType,Depth,Self,['Cons', A, B ],['Cons', AA, BB]):- no_cons_reduce, !,
  eval_args(RetType,Depth,Self,A,AA), eval_args(RetType,Depth,Self,B,BB).
...
eval_args20(RetType,Depth,Self,['change-state!',StateExpr, UpdatedValue], Ret):- !, eval_args(RetType,Depth,Self,StateExpr,StateMonad),
  eval_args(RetType,Depth,Self,UpdatedValue,Value),  'change-state!'(Depth,Self,StateMonad, Value, Ret).
eval_args20(RetType,Depth,Self,['new-state',UpdatedValue],StateMonad):- !,
  eval_args(RetType,Depth,Self,UpdatedValue,Value),  'new-state'(Depth,Self,Value,StateMonad).
eval_args20(RetType,Depth,Self,['get-state',StateExpr],Value):- !,
  eval_args(RetType,Depth,Self,StateExpr,StateMonad), 'get-state'(StateMonad,Value).
...
eval_args20(RetType,Depth,Self,['get-type',Val],Type):- !, get_type1(Depth,Self,Val,Type),ground(Type),Type\==[], Type\==Val,!.
/*
...
get_type1(Depth,Self,EvalMe,Type):- needs_eval(EvalMe),eval_args(RetType,Depth,Self,EvalMe,Val), \+ needs_eval(Val),!,
   get_type1(Depth,Self,Val,Type).
...
get_type1(Depth,Self,Expr,Type):-Depth2 is Depth-1, eval_args(RetType,Depth2,Self,Expr,Val),Expr\=@=Val,get_type1(Depth2,Self,Val,Type).
...
eval_args20(RetType,Depth,Self,['length',L],Res):- !, eval_args(RetType,Depth,Self,L,LL), !, (is_list(LL)->length(LL,Res);Res=1).
eval_args20(RetType,Depth,Self,['CountElement',L],Res):- !, eval_args(RetType,Depth,Self,L,LL), !, (is_list(LL)->length(LL,Res);Res=1).
...
*/
eval_args20(RetType,_Dpth,Self,['import!',Other,File],RetVal):- into_space(Self,Other,Space),!, include_metta(Space,File),!,return_empty(Space,RetVal). %RetVal=[].
eval_args20(RetType,Depth,Self,['bind!',Other,Expr],RetVal):-
   into_name(Self,Other,Name),!,eval_args(RetType,Depth,Self,Expr,Value),nb_setval(Name,Value),  return_empty(Value,RetVal).
eval_args20(RetType,Depth,Self,['pragma!',Other,Expr],RetVal):-
   into_name(Self,Other,Name),!,eval_args(RetType,Depth,Self,Expr,Value),set_option_value(Name,Value),  return_empty(Value,RetVal).
eval_args20(RetType,_Dpth,Self,['transfer!',File],RetVal):- !, include_metta(Self,File),  return_empty(Self,RetVal).



eval_args20(RetType,Depth,Self,['nop',Expr],Empty):- !,  eval_args(RetType,Depth,Self,Expr,_), return_empty([],Empty).
...
eval_args20(RetType,Depth,Self,['and',X,Y],TF):- !, as_tf((eval_args(RetType,Depth,Self,X,'True'),eval_args(RetType,Depth,Self,Y,'True')),TF).
eval_args20(RetType,Depth,Self,[And,X|Y],TF):- is_and(And),!,eval_args(RetType,Depth,Self,X,TF1),
...
eval_args20(RetType,_Dpth,Self,['add-atom',Other,PredDecl],TF):- !, into_space(Self,Other,Space), as_tf(do_metta(Space,load,PredDecl),TF).
eval_args20(RetType,_Dpth,Self,['remove-atom',Other,PredDecl],TF):- !, into_space(Self,Other,Space), as_tf(do_metta(Space,unload,PredDecl),TF).
eval_args20(RetType,_Dpth,Self,['atom-count',Other],Count):- !, into_space(Self,Other,Space), findall(_,metta_defn(Other,_,_),L1),length(L1,C1),findall(_,metta_atom(Space,_),L2),length(L2,C2),Count is C1+C2.
eval_args20(RetType,_Dpth,Self,['atom-replace',Other,Rem,Add],TF):- !, into_space(Self,Other,Space), copy_term(Rem,RCopy),
  as_tf((metta_atom_iter_ref(Space,RCopy,Ref), RCopy=@=Rem,erase(Ref), do_metta(Other,load,Add)),TF).


eval_args20(RetType,Depth,Self,['+',N1,N2],N):- number(N1),!,
   eval_args(RetType,Depth,Self,N2,N2Res), catch(N is N1+N2Res,_E,(set_last_error(['Error',N2Res,'Number']),fail)).
eval_args20(RetType,Depth,Self,['-',N1,N2],N):- number(N1),!,
   eval_args(RetType,Depth,Self,N2,N2Res), catch(N is N1-N2Res,_E,(set_last_error(['Error',N2Res,'Number']),fail)).

eval_args20(RetType,Depth,Self,[V|VI],[V|VO]):- nonvar(V),is_metta_data_functor(V),is_list(VI),!,maplist(eval_args(RetType,Depth,Self),VI,VO).

eval_args20(RetType,Depth,Self,X,Y):-
  (eval_args40(RetType,Depth,Self,X,Y)*->true;
    (eval_args2_failed(RetType,Depth,Self,X,Y)*->true;X=Y)).


eval_args2_failed(RetType,_Dpth,_Slf,T,TT):- T==[],!,TT=[].
eval_args2_failed(RetType,_Dpth,_Slf,T,TT):- var(T),!,TT=T.
eval_args2_failed(RetType,_Dpth,_Slf,[F|LESS],Res):- once(eval_selfless([F|LESS],Res)),mnotrace([F|LESS]\==Res),!.
%eval_args2_failed(RetType,Depth,Self,[V|Nil],[O]):- Nil==[], once(eval_args(RetType,Depth,Self,V,O)),V\=@=O,!.
eval_args2_failed(RetType,Depth,Self,[H|T],[HH|TT]):- !,
  eval_args(RetType,Depth,Self,H,HH),
  eval_args2_failed(RetType,Depth,Self,T,TT).

eval_args2_failed(RetType,Depth,Self,T,TT):- eval_args(RetType,Depth,Self,T,TT).
...
% Kills zero arity functions eval_args20(RetType,Depth,Self,[X|Nil],[Y]):- Nil ==[],!,eval_args(RetType,Depth,Self,X,Y).
...
eval_args40(RetType,Depth,Self,[H|PredDecl],Res):- mnotrace(is_user_defined_head(Self,H)),!,
   eval_args30(RetType,Depth,Self,[H|PredDecl],Res).

% function inherited by system
eval_args40(RetType,Depth,Self,PredDecl,Res):- eval_args40(RetType,Depth,Self,PredDecl,Res).
...
%eval_args40(RetType,Depth,Self,PredDecl,Res):- eval_args3(RetType,Depth,Self,PredDecl,Res).
...
eval_args30(RetType,Depth,Self,H,B):-
   (eval_args34(RetType,Depth,Self,H,B)*->true;eval_args67(RetType,Depth,Self,H,B)).

eval_args34(RetType,_Dpth,Self,H,B):-  (metta_defn(Self,H,B);(metta_atom(Self,H),B='True')).
...
eval_args67(RetType,Depth,Self,[H1|Args],Res):-
   ...
   eval_args30(RetType,Depth,Self,[H1|NewArgs],Res).
...
eval_args67(RetType,Depth,Self,[F|PredDecl],Res):-
   ...
   eval_args(RetType,Depth,Self,SSub,Repl),
   mnotrace((SSub\=Repl,subst(PredDecl,SSub,Repl,Temp))),
   eval_args30(RetType,Depth,Self,[F|Temp],Res).

%eval_args67(RetType,Depth,Self,X,Y):- (eval_args38(RetType,Depth,Self,X,Y)*->true;metta_atom_iter(Depth,Self,[=,X,Y])).
...
eval_args38(RetType,_Dpth,Self,[H|_],_):- mnotrace( \+ is_user_defined_head_f(Self,H) ), !,fail.
eval_args38(RetType,_Dpth,Self,[H|T1],Y):- metta_defn(Self,[H|T1],Y).
eval_args38(RetType,_Dpth,Self,[H|T1],'True'):- metta_atom(Self,[H|T1]).
eval_args38(RetType,_Dpth,Self,CALL,Y):- fail,append(Left,[Y],CALL),metta_defn(Self,Left,Y).


%eval_args3(RetType,Depth,Self,['ift',CR,Then],RO):- fail, !, %fail, % trace,
...
eval_args40(RetType,Depth,Self,[F|X],FY):- is_function(F), \+ is_special_op(F), is_list(X),
  maplist(eval_args(RetType,Depth,Self),X,Y),!,eval_args5(RetType,Depth,Self,[F|Y],FY).
eval_args40(RetType,Depth,Self,FX,FY):- eval_args5(RetType,Depth,Self,FX,FY).

eval_args5(RetType,_Dpth,_Slf,[F|LESS],Res):- once(eval_selfless([F|LESS],Res)),mnotrace(([F|LESS]\==Res)),!.
