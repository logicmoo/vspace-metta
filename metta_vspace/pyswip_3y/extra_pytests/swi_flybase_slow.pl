...
load_flybase:- recount_total_loaded_atoms,!,load_flybase_files,fb_stats.
load_flybase_dirs:- 
...
load_flybase_files2:-  % 47 tables
  with_option_value(use_va,t,load_flybase('./precomputed_files/genes/fbgn_exons2affy1_overlaps.tsv')),
  with_option_value(use_va,t,load_flybase('./precomputed_files/genes/fbgn_exons2affy2_overlaps.tsv')),
  load_flybase('./precomputed_files/*'),
  !.

load_flybase_files3:-  % 47 tables
  with_option_value(use_va,t,load_flybase('./data/ftp.flybase.net/releases/current/precomputed_files/genes/fbgn_exons2affy1_overlaps.tsv')),
  with_option_value(use_va,t,load_flybase('./data/ftp.flybase.net/releases/current/precomputed_files/genes/fbgn_exons2affy2_overlaps.tsv')),
...
load_fb_cache(_File,OutputFile,_DataPred):- exists_file(OutputFile),!,ensure_loaded(OutputFile),!.
load_fb_cache(File,_OutputFile,_PredName):- load_files([File],[qcompile(large)]).
...
load_flybase0(Ext,File):-  
  ...
  data_pred(Name,DataPred), 
  load_flybase(Ext,File,OutputFile,DataPred).
...
%load_flybase(_Ext,_File,OutputFile,_PredName):- exists_file(OutputFile),size_file(OutputFile,N),N>100,!.

:- dynamic(load_state/2).
load_flybase(_Ext,File,_OutputFile,_DataPred):- load_state(File,_),!.
load_flybase(Ext,File,OutputFile,DataPred):- file_to_sep(File,Sep),!,
  assert(load_state(File,loading)),
  extreme_debug(fbug(load_flybase(Ext,File,OutputFile,DataPred))),
  setup_call_cleanup(open(File,read,Stream),
       setup_call_cleanup(open(OutputFile,write,OutputStream,[encoding(utf8)]),
           %load_flybase_sv(Sep,File,Stream,OutputStream,DataPred),
           load_flybase(Sep,File,Stream,OutputStream,DataPred),
    close(OutputStream)),
  close(Stream)),!,
...
%load_flybase(Ext,File,OutputFile,DataPred):-  Ext==obo,!,load_fb_obo(Ext,File,OutputFile,DataPred).
%load_flybase(Ext,File,OutputFile,DataPred):-  Ext==json,!,load_fb_json(Ext,File,OutputFile,DataPred).
load_flybase(Ext,File,OutputFile,DataPred):- fbug(load_flybase(Ext,File,OutputFile,DataPred)),!.


load_fb_json(Ext,File,OutputFile,DataPred):- fbug(load_fb_json(Ext,File,OutputFile,DataPred)).
load_fb_obo(Ext,File,OutputFile,DataPred):- fbug(load_fb_obo(Ext,File,OutputFile,DataPred)),!.
%load_fb_obo(Ext,File,OutputFile,DataPred):- fbug(load_fb_obo(Ext,File,OutputFile,DataPred)),
...
load_flybase(Sep,File,Stream,OutputStream,DataPred):- 
 must_det_ll((
  ...
  atomic_list_concat([data,DataPred],'_',DataPred0),
  data_pred(DataPred0,DataPred),
  load_flybase_sv(Sep,File,Stream,OutputStream,DataPred))).

% Sep,File,Stream,OutputStream,DataPred
load_flybase_sv(Sep,File,Stream,OutputStream,DataPred):- at_end_of_stream(Stream),!,
  once(load_fb_data(_ArgTypes,File,Stream,DataPred,Sep,end_of_file,OutputStream)).
load_flybase_sv(Sep,File,Stream,OutputStream,DataPred):- 
  flag(loaded_from_file,_,0),
  time((repeat,
  read_line_to_chars(Stream, Chars),  
  once(load_flybase_chars(ArgTypes,File,Stream,DataPred,Sep,Chars,OutputStream)),
  once(done_reading(File);at_end_of_stream(Stream)),!,
  once(load_fb_data(ArgTypes,File,Stream,DataPred,Sep,end_of_file,OutputStream)))),
  flag(loaded_from_file,X,X),!,
  fb_stats(DataPred),
  pl_stats(File,X).


%save_conversion_data(ArgTypes,DataPred,OutputStream,Data):- maplist(write_flybase_data(ArgTypes,ArgTypes,DataPred,OutputStream),Data).
...
load_flybase_chars(_ArgTypes,File,_Stream,_DataPred,Sep,Chars,_OutputStream):- 
...
load_flybase_chars(ArgTypes,File,Stream,DataPred,Sep,Chars,OutputStream):- is_swipl,
  name(Sep,[SepCode]),
  ...
  HeaderRow=..[_|Header],
  fix_header_names(DataPred,Header,ArgTypes),
  (is_really_header_row(Header,ArgTypes)
    -> (fbug(t_h_n(DataPred,Header,ArgTypes)),fb_assert(t_h_n(DataPred,Header,ArgTypes)))
     ; (fbug('NO_HEADER'(DataPred,Header,ArgTypes)),
        write_flybase_data(ArgTypes,OutputStream,DataPred,Header))),

%  forall(table_columns(File,ColInfo),pp_ilp(table_columns(File,ColInfo))),
%  forall(table_columns(DataPred,ColInfo),pp_ilp(table_columns(DataPred,ColInfo))),
  nop(once((table_columns(File,ArgTypes);table_columns(DataPred,ArgTypes)))),
  load_fb_data(ArgTypes,File,Stream,DataPred,Sep,is_swipl,OutputStream).


fix_header_names(DataPred,Header,GNames):- 
   maplist(fix_header_names(Header,DataPred),Header,ArgTypes),
   include( \=(''),ArgTypes,GNames).
   

load_fb_data(_ArgTypes,File,_Stream,_DataPred,_Sep,Data,_OutputStream):-  
   ...
   repeat, 
     ...
     flag(loaded_from_file,X,X), 
      (((RData== end_of_file);(X>Max)) -> assert(done_reading(File)) ; 
       (RData =..[_|Data], 
       once(write_flybase_data(ArgTypes,OutputStream,DataPred,Data)),fail)),!.
...
load_fb_data(ArgTypes,File,Stream,DataPred,Sep, is_swipl,OutputStream):- 
  ...
  csv_options(CompiledOptions,[strip(true),convert(true),separator(SepCode)]),
   (option_value(max_per_file,Max)->true;Max=inf),
     ...
     flag(loaded_from_file,X,X), 
      (((RData== end_of_file);(X>Max)) -> assert(done_reading(File)) ; 
       (RData =..[_|Data], once(write_flybase_data(ArgTypes,OutputStream,DataPred,Data)),
         load_fb_data(ArgTypes,File,Stream,DataPred,Sep, is_swipl,OutputStream))),!.
...
fb_stats:- gc_now,
   ...
   nop((pl_stats('Bytes Per Atom (Average)',PA), pl_stats('Bytes Per ConceptNode (Average)',ACS))),
   nop((pl_stats('Relational Memory',RM), pl_stats('ConceptNode Memory',AS))), 
...
write_flybase_data(_ArgTypes,_OutputStream,_DataPred,[]):-!.
write_flybase_data(ArgTypes,OutputStream,DataPred,Data):-
  has_list(ArgTypes),
  fix_list_args(DataPred,ArgTypes,Data,NewData),!,
  write_flybase_data1(ArgTypes,Data,OutputStream,DataPred,NewData).
write_flybase_data(ArgTypes,OutputStream,DataPred,NewData):-
  write_flybase_data1(ArgTypes,NewData,OutputStream,DataPred,NewData),!.

write_flybase_data1(ArgTypes,OldData,OutputStream,DataPred,DataL):-
  ignore((
   ...
   (fb_pred(F,A)-> true; (dynamic(F/A),assert(fb_pred(F,A)))),    
    ...
    assert(Data),
    ignore((((has_list(ArgTypes)->(X<23,X>20); (X<13,X>10)); (X>0,(0 is X rem 500_000),fb_stats)),fbug(X=Data),ignore((fail,OldData\==Data,fbug(oldData=OldData))))),
    catch_ignore(ignore((X<1000,must_det_ll((display(OutputStream,Data),writeln(OutputStream,'.')))))))),!.

into_datum(DataPred,[D|DataL],Data):-
  (option_value(use_va,t) -> Data=..[DataPred,D,DataL]; Data=..[DataPred,D|DataL]).

make_assertion(ArgTypes,DataPred,DataL0,Data,DataL):-
 must_det_ll((
    into_datum(DataPred,DataL0,Data0),
    Data0=..[F|Args],
    nop(if_t(var(ArgTypes),must_det_ll((once((length(Args,Len),length(ArgTypes,Len),once((table_columns(DataPred,ArgTypes);table_columns(F,ArgTypes))))))))),
    fix_list_args(DataPred,ArgTypes,Args,DataL), 
    into_datum(F,DataL,Data))).


%fix_list_args(_,Y,Y):- \+ current_prolog_flag(fb_full,true),!.
fix_list_args(_DataPred,_ArgTypes,Args,Args):-!.
fix_list_args(DataPred,ArgTypes,Args,NewArgs):-
  Args = [D|_],  
  fix_concept(D,Term),  
  fix_elist_args(Term,DataPred,1,ArgTypes,Args,NewArgs),
  extreme_debug(ignore(((Args \== NewArgs,fbug(NewArgs))))).


fix_elist_args(_Term,_DataPred,_N,_,X,X).
fix_elist_args(Term,DataPred,N,[listOf(Type)|ArgTypes],[Arg|Args],[New|NewArgs]):- nonvar(Type), !, as_list([],Arg,New),assert_type_of(Term,DataPred,N,Type,New),!, N2 is N +1, fix_elist_args(Term,DataPred,N2,ArgTypes,Args,NewArgs).
fix_elist_args(Term,DataPred,N,[listOf(Type,Seps)|ArgTypes],[Arg|Args],[New|NewArgs]):- nonvar(Type), !, as_list(Seps,Arg,New),assert_type_of(Term,DataPred,N,Type,New),!, N2 is N +1, fix_elist_args(Term,DataPred,N2,ArgTypes,Args,NewArgs).
fix_elist_args(Term,DataPred,N,[Type|ArgTypes],[Arg|Args],[Arg|NewArgs]):- !, assert_type_of(Term,DataPred,N,Type,Arg),!, N2 is N +1, fix_elist_args(Term,DataPred,N2,ArgTypes,Args,NewArgs).
fix_elist_args(_Term,_DataPred,_N,_,X,X).

assert_type_of(Term,DataPred,N,Type,Arg):- is_list(Arg),!,maplist(assert_type_of(Term,DataPred,N,Type),Arg).
assert_type_of(Term,DataPred,N,Type,Arg):- 
   assert(type_of(Arg,colFn(DataPred,N))),
   assert(table_arg(Term,N,Arg,DataPred)),
   if_t(nonvar(Type),assert(type_of(Arg,Type))).
...
as_list(SepL,A,ListO):- atom(A), member(Sep,SepL),catch_ignore(atomic_list_concat(List,Sep,A)),List\=[_],!,maplist(fix_concept,List,ListO).
as_list(_,A,ListO):- atom(A), member(Sep,['|',',',';']),catch_ignore(atomic_list_concat(List,Sep,A)),List\=[_],!,maplist(fix_concept,List,ListO).
as_list(_Sep,A,[AO]):- fix_concept(A,AO).

fix_concept(A,A):- \+ atom(A),!.
fix_concept(A,AO):-  atom_concat('flybase:',AM,A),fix_concept(AM,AO),!.
...
is_good_atom_name(E1):- atom_length(E1,L),L>5.
...
mine_typelevel_overlaps:- 
  forall(mine_typelevel_overlaps(E1,SC1,SC2),pp_fb(E1=[SC1,SC2])).

mine_typelevel_overlaps(E1,nth1(N2,F2),nth1(N1,F1)):-
  table_columns(F1,P1), table_columns(F2,P2),
  F1@>F2, nth1(N1,P1,E1),
  good_concept(E1),nth1(N2,P2,E1).

mine_atomspace_overlaps:-
  fb_two_preds(F1,N1,A1,F2,N2,A2),
  once((functor(P1,F1,A1),functor(P2,F2,A2),
  call(P1), arg(N1,P1,E1),good_concept(E1), arg(N2,P2,E1),call(P2))),
  ignore(table_column_type(F1,N1,T1)),
  ignore(table_column_type(F2,N2,T2)),
  pp_fb(E1=[P1,P2,arg(T1,N1,F1/A1)+arg(T2,N2,F2/A2)]),
  assert(maybe_corisponds(arg(T1,N1,F1/A1),arg(T2,N2,F2/A2))).

fb_two_preds(F1,N1,A1,F2,N2,A2):- 
  fb_pred(F1,A1), fb_pred(F2,A2), F1@>F2,
  between(1,A1,N1),between(1,A2,N2).
  
fb_two_preds(F1,N1,A1,F2,N2,A2):- 
  fb_pred(F1,A1), fb_pred(F2,A2),F1@>F2,
  mine_typelevel_overlaps(_,nth1(N2,F2),nth1(N1,F1)).

table_column_type(F2,N2,T2):- table_columns(F2,List),nth1(N2,List,T2).
...
fix_header_names(FL,DataPred,ID,Out):- member(RF,['#',' ','_']),atom_concat(MID,RF,ID),!,fix_header_names(FL,DataPred,MID,Out).
fix_header_names(FL,DataPred,ID,Out):- member(RF,['#',' ','_','_id','_ID']),atom_concat(MID,RF,ID),!,fix_header_names(FL,DataPred,MID,Out).
fix_header_names(FL,DataPred,ID,Out):- member(RF,['#',' ','_']),atom_concat(RF,MID,ID),!,fix_header_names(FL,DataPred,MID,Out).
fix_header_names(FL,DataPred,ID,Out):- member(RF,['__',' ']),atomic_list_concat(MIDL,RF,ID),MIDL\=[_],atomic_list_concat(MIDL,'_',MID),!,
   fix_header_names(FL,DataPred,MID,Out).
fix_header_names(FL,DataPred,ID,listOf(AOut)):- member(RF,['(es)','(s)','ids']),atomic_list_concat([Left,Right],RF,ID),atomic_list_concat([Left,Right],'_',MID),!,
   fix_header_names(FL,DataPred,MID,AOut),!. % atom_concat('ListOf_',AOut,Out),!.
fix_header_names(_,_,Name,Name):- \+ too_generic(Name),!.
fix_header_names(_,_,Name,Name):- atomic_list_concat([_,_|_],'_',Name),!.
%fix_header_names(_,DataPred,ID,Out):- atomic_list_concat([DataPred,ID],'_column_',Out).
%fix_header_names(FieldList,DataPred,ID,Out):- atomic_list_concat([DataPred,ID],'_',Out), \+ member(Out,FieldList).
