...
eval_00(Eq,RetType,Depth,Self,X,YO):-
  Depth2 is Depth-1,
  copy_term(X, XX),
  eval_15(Eq,RetType,Depth,Self,X,M),
  ((M\=@=XX, \+ self_eval(M))->
...
%eval_11(Eq,RetType,_Dpth,_Slf,X,Y):- self_eval(X),!,Y=X.
...
eval_20(Eq,RetType,_Dpth,Self,['atom-count',Other],Count):- !, into_space(Self,Other,Space), findall(_,metta_defn(Other,_,_),L1),length(L1,C1),findall(_,metta_atom(Space,_),L2),length(L2,C2),Count is C1+C2.
...
eval_20(Eq,RetType,Depth,Self,['add-atom',Other,PredDecl],Res):- !,
   into_space(Depth,Self,Other,Space),
   do_metta(python,load,Space,PredDecl,TF),return_empty([],Res),check_returnval(Eq,RetType,TF).
eval_20(Eq,RetType,Depth,Self,['remove-atom',Other,PredDecl],Res):- !,   into_space(Depth,Self,Other,Space),
   do_metta(python,unload,Space,PredDecl,TF),return_empty([],Res),check_returnval(Eq,RetType,TF).
eval_20(Eq,RetType,Depth,Self,['atom-count',Other],Count):- !,   (( into_space(Depth,Self,Other,Space), findall(_,metta_defn(Other,_,_),L1),length(L1,C1),
    findall(_,metta_atom(Space,_),L2),length(L2,C2),Count is C1+C2)),check_returnval(Eq,RetType,Count).
eval_20(Eq,RetType,Depth,Self,['atom-replace',Other,Rem,Add],TF):- !,
 ((into_space(Depth,Self,Other,Space), copy_term(Rem,RCopy),
   as_tf((metta_atom_iter_ref(Space,RCopy,Ref), RCopy=@=Rem,erase(Ref), do_metta(Other,load,Add)),TF))),
 check_returnval(Eq,RetType,TF).
eval_20(Eq,RetType,Depth,Self,['get-atoms',Other],Atom):- !,
  ignore(RetType='Atom'),
  get_atoms(Depth,Self,Other,Atom), check_returnval(Eq,RetType,Atom).
...
%eval_20(Eq,RetType,Depth,Self,['match',Other,Goal,Template],Template):- into_space(Self,Other,Space),!, metta_atom_iter(Depth,Space,Goal).
%eval_20(Eq,RetType,Depth,Self,['match',Other,Goal,Template,Else],Template):- into_space(Self,Other,Space),!,  (metta_atom_iter(Depth,Space,Goal)*->true;Else=Template).

% Match-ELSE
eval_20(Eq,RetType,Depth,Self,['match',Other,Goal,Template,Else],Template):- !,
  ((eval_20(Eq,RetType,Depth,Self,['match',Other,Goal,Template],Template),
       \+ return_empty([],Template))*->true;Template=Else).
% Match-TEMPLATE

eval_20(Eq,RetType,Depth,Self,['match',Other,Goal,Template],Res):- !,
  dcall(( % copy_term(Goal+Template,CGoal+CTemplate),
  catch(try_match(Eq,RetType,Depth,Self,Other,Goal,Template,Res),E,
   ((wdmsg(catch(try_match(Eq,RetType,Depth,Self,Other,Goal,Template,Res))=E)),
     rtrace(try_match(Eq,RetType,Depth,Self,Other,Goal,Template,Res)))))),
  %print(Template),
    finish_eval(Depth,Self,Template,Res).



try_match(Eq,RetType,Depth,Self,Other,Goal,Template,Res):- !,
 % into_space(Depth,Self,Other,Space),
  metta_atom_iter(Depth,Self,Other,Goal),
  eval_99(Eq,RetType,Depth,Self,Template,Res).
...
metta_atom_iter(_Depth,_Slf,Other,[Equal,[F|H],B]):- '=' == Equal,!,  % trace,
   dcall(metta_defn(Other,[F|H],B)). % once(eval_until_unify(Eq,RetType,Depth,Other,H,HH)).

%metta_atom_iter(Depth,_Slf,Other,[Equal,[F|H],B]):- '=' == Equal,!,  % trace,
 %  dcall(metta_defn(Other,[F|HH],B)), once(eval_until_unify(Eq,RetType,Depth,Other,H,HH)).
...
eval_20(Eq,RetType,Depth,Self,['superpose',List],Res):- !,
  ((is_user_defined_goal(Self,List) ,eval(Eq,RetType,Depth,Self,List,UList), List\=@=UList)
...
eval_20(Eq,RetType,Depth,Self,['pragma!',Other,Expr],RetVal):-
   must_det_ll((into_name(Self,Other,Name),!,nd_ignore((eval(Eq,RetType,Depth,Self,Expr,Value),set_option_value(Name,Value))),  return_empty(Value,RetVal),
    check_returnval(Eq,RetType,RetVal))).
...
eval_40(Eq,RetType,Depth,Self,['+',N1,N2],N):- number(N1),!,
   eval(Eq,RetType,Depth,Self,N2,N2Res), notrace(catch(N is N1+N2Res,_E,(set_last_error(['Error',N2Res,'Number']),fail))).
eval_40(Eq,RetType,Depth,Self,['-',N1,N2],N):- number(N1),!,
   eval(Eq,RetType,Depth,Self,N2,N2Res), notrace(catch(N is N1-N2Res,_E,(set_last_error(['Error',N2Res,'Number']),fail))).
eval_40(Eq,RetType,Depth,Self,['*',N1,N2],N):- number(N1),!,
   eval(Eq,RetType,Depth,Self,N2,N2Res), notrace(catch(N is N1*N2Res,_E,(set_last_error(['Error',N2Res,'Number']),fail))).
...
eval_60(Eq,RetType,Depth,Self,H,B):-
   (eval_64(Eq,RetType,Depth,Self,H,B)*->true;eval_67(Eq,RetType,Depth,Self,H,B)).


%eval_64(Eq,_RetType,_Dpth,Self,H,B):-  Eq='=',!, metta_defn(Self,H,B).
eval_64(Eq,_RetType,_Dpth,Self,H,B):-  Eq='match', dcall(metta_atom(Self,H)),B=H.
eval_64(Eq,_RetType,_Depth,Self,[H|Args],B):- % no weird template matchers
...
eval_64(Eq,_RetType,_Depth,Self,[H|Args],B):-
    Eq='=',!,
...
% Has subterm to eval
eval_67(Eq,RetType,Depth,Self,[F|PredDecl],Res):-
   Depth>1,
...
%eval_68(Eq,RetType,_Dpth,Self,[H|T1],Y):- metta_defn(Self,[H|T1],Y).
%eval_68(Eq,RetType,_Dpth,Self,[H|T1],'True'):- metta_atom(Self,[H|T1]).
%eval_68(Eq,RetType,_Dpth,Self,CALL,Y):- fail,append(Left,[Y],CALL),metta_defn(Self,Left,Y).


%eval_6(Depth,Self,['ift',CR,Then],RO):- fail, !, %fail, % trace,
%   metta_defn(Self,['ift',R,Then],Become),eval(Eq,RetType,Depth,Self,CR,R),eval(Eq,RetType,Depth,Self,Then,_True),eval(Eq,RetType,Depth,Self,Become,RO).
