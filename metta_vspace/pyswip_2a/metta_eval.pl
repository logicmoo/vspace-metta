...
eval_args20(Depth,Self,[V|VI],[V|VO]):- is_metta_data_functor(V), !, must_eval(adjust_args(Depth,Self,V,VI,VO)).
...
eval_args30(Depth,Self,['match',Other,Goal,Template],Res):- into_space(Self,Other,Space),!,
  (metta_atom_iter(Depth,Space,Goal)*->eval_args99(Depth,Self,Template,Res);(fail,return_empty([],Template))).
...
eval_args30(Depth,Self,[H|Params],Res):-
   mnotrace(is_user_defined_head(Self,H)),!,
   %must_eval(adjust_args(Depth,Self,H,Params,ParamsO)),
   eval_args60(Depth,Self,[H|Params],Res).
...
eval_args60(Depth,Self,['CountElement',L],Res):- !, eval_args(Depth,Self,L,LL), !, (is_list(LL)->length(LL,Res);Res=1).
eval_args60(Depth,Self,['do',Expr],Empty):- !,  eval_args(Depth,Self,Expr,_), return_empty([],Empty).
...
eval_args60(_Dpth,_Slf,['make_list',List],MettaList):- !, into_metta_cons(List,MettaList).

eval_args60(Depth,Self,['If',Cond,Then],Res):- is_user_defined_head_f(Self,'If'), !,
   eval_args(Depth,Self,Cond,TF),
   (is_True(TF) -> eval_args(Depth,Self,Then,Res) ; Res = []).

eval_args60(Depth,Self,['If',Cond,Then,Else],Res):- is_user_defined_head_f(Self,'If'), !,
   eval_args(Depth,Self,Cond,TF),
   (is_True(TF) -> eval_args(Depth,Self,Then,Res);eval_args(Depth,Self,Else,Res)).

% @TODO: This should not be here
eval_args60(_Dpth,_Slf,['ift',Cond,Then],Then):- Cond == 'True', !.


eval_args60(Depth,Self,H,B):-
